...
def display_density(density):
    # Show the spatial distribution within this depth, regardless of ray count.
    values = np.zeros(density.shape, dtype=np.float64)
    occupied = density > 0
    if not np.any(occupied):
        return values

    probability = density[occupied].astype(np.float64)
    probability /= probability.sum()
    log_probability = np.log(probability)
    low = log_probability.min()
    high = np.percentile(log_probability, 99.5)
    if high == low:
        high = log_probability.max()
    if high == low:
        values[occupied] = 1
    else:
        # Keep every occupied pixel visible, including low-probability regions.
        values[occupied] = 0.25 + 0.75 * np.clip(
            (log_probability - low) / (high - low), 0, 1
        )
    return values


def save_analysis_figure(image, hits, throughput, output_path):
    reference = np.clip(image.numpy(), 0, 1)
    height, width = hits.shape[1:]
    dpi = 200
    gap = 20
    left = 70
    bottom = 45
    ref_width, ref_height = width // 2, height // 2
    figure_width = left + ref_width + gap + 5 * width + 4 * gap + 20
    figure_height = bottom + 2 * height + gap + 55
    fig = plt.figure(figsize=(figure_width / dpi, figure_height / dpi), dpi=dpi)

    # Give each density map one output pixel per analysis pixel.
    ref_bottom = bottom + (2 * height + gap - ref_height) // 2
    ref_ax = fig.add_axes((left / figure_width, ref_bottom / figure_height,
                           ref_width / figure_width, ref_height / figure_height))
    ref_ax.imshow(reference, interpolation="nearest")
    ref_ax.set_title("Ref", pad=3)
    ref_ax.axis("off")

    fig.text((left + ref_width + gap) / figure_width, 17 / figure_height,
             "Each depth uses its own color scale; darkest pixels have no hits.",
             ha="left", va="center", fontsize=9)

    for row, (label, density) in enumerate((("Ray Hit Density", hits),
                                           ("Throughput Density", throughput))):
        y = bottom + (height + gap if row == 0 else 0)
        fig.text(25 / figure_width, (y + height / 2) / figure_height, label,
                 rotation=90, ha="center", va="center", fontsize=10)
        for slot in range(density.shape[0]):
            x = left + ref_width + gap + slot * (width + gap)
            ax = fig.add_axes((x / figure_width, y / figure_height,
                               width / figure_width, height / figure_height))
            ax.imshow(display_density(density[slot]), cmap="magma", vmin=0,
                      vmax=1, origin="upper", interpolation="nearest")
            if row == 0:
                ax.set_title(f"d={slot + 1}", pad=3)
            ax.axis("off")
...
if __name__ == "__main__":
    ...
    for scene_path in scenes:
        ...
        shape = (integrator.analysis_depth, integrator.analysis_height, integrator.analysis_width)
        hits = integrator.hit_density.numpy().reshape(shape)
        throughput = integrator.throughput_density.numpy().reshape(shape)
        valid_hits = integrator.valid_hits.numpy()

        for slot in range(integrator.analysis_depth):
            projected_hits = hits[slot].sum(dtype=np.uint64)
            print(
                f"d={slot + 1}: valid hits={valid_hits[slot]}, "
                f"projected hits={projected_hits}, "
                f"hit density sum={projected_hits}, "
                f"throughput density sum={throughput[slot].sum(dtype=np.float64):.6g}, "
                f"nonzero pixels={np.count_nonzero(hits[slot])}"
            )
        save_analysis_figure(image, hits, throughput, Path("results") / f"{name}.png")
